...
type CourseGroup = { day: number; start: number; end: number; courses: Course[] };
...
export default function Home() {
  ...
  const [courseChoices, setCourseChoices] = useState<Course[] | null>(null);
  ...
  const courseGroups = useMemo(() => {
    ...
    for (let day = 1; day <= 5; day += 1) {
      const dayCourses = courses.filter((course) => course.day === day).sort((a, b) => a.start - b.start || a.end - b.end);
      for (const course of dayCourses) {
        const prior = groups.at(-1);
        if (prior?.day === day && course.start <= prior.end) {
          prior.courses.push(course);
          prior.end = Math.max(prior.end, course.end);
        } else groups.push({ day, start: course.start, end: course.end, courses: [course] });
      }
    }
    return groups;
  }, [courses]);
  ...
  return <div className="app-shell">
    ...
    <main id="top">
      ...
      <section className="status-grid" id="status"><article className="status-card current-card"><div className="status-label"><span><Clock3 size={16} /> 當下課程</span><span className="now-badge">NOW</span></div>{live.current ? <><h2>{live.current.name}</h2><div className="room-line"><MapPin size={18} /> {live.current.room} · {live.current.className}</div><div className="progress-track"><span style={{ width: `${Math.max(8, 100 - live.remaining * 2)}%` }} /></div><p className="status-note">距離下課還有 <strong>{live.remaining} 分鐘</strong></p></> : <div className="empty-current"><span>☕</span><div><h2>目前無課程</h2><p>休息一下，下一堂課資訊在<span className="desktop-direction">右側</span><span className="mobile-direction">下方</span>。</p></div></div>}</article><article className="status-card next-card"><div className="status-label"><span><CalendarDays size={16} /> 下一節課</span></div>{live.next ? <><div className="next-time"><strong>{live.next.daysAhead === 0 ? '今天' : weekdays[live.next.course.day - 1]}</strong><span>{periods[live.next.course.start - 1].start}</span></div><h2>{live.next.course.name}</h2><div className="room-line"><MapPin size={18} /> {live.next.course.room} · {live.next.course.className}</div></> : <h2>本週沒有更多課程</h2>}</article></section>

      <section className="schedule-section" id="schedule">
        <div className="section-heading"><div><span className="eyebrow">WEEKLY SCHEDULE</span><h2>本週課表</h2></div><div className="legend"><span><i className="legend-dot mint" />班級課程</span><span>共 {courses.length} 筆課程</span></div></div>
        <div className="schedule-scroll"><div className="schedule-grid">
          <div className="grid-corner">時間</div>
          {weekdays.map((day, index) => <div className={`day-head ${now.getDay() === index + 1 ? 'today' : ''}`} key={day}><span>{day}</span><small>{getDateLabel(index + 1)}</small></div>)}
          {periods.map((period) => <div className="grid-row" key={period.id}>
            <div className="time-cell"><strong>{period.id}</strong><span>{period.start}<br />{period.end}</span></div>
            {weekdays.map((_, dayIndex) => {
              const group = courseGroups.find((item) => item.day === dayIndex + 1 && item.start === period.id);
              if (!group) return <div className="slot" key={dayIndex} />;
              const height = `calc(${group.end - group.start + 1} * var(--row-height) - 8px)`;
              if (group.courses.length === 1) {
                const course = group.courses[0];
                return <div className="slot" key={dayIndex}><button className={`course-card ${course.color}`} style={{ height }} onClick={() => setDetail(course)}><strong>{course.name}</strong><span><MapPin size={13} /> {course.room}</span></button></div>;
              }
              return <div className="slot" key={dayIndex}><button className="course-card conflict-card" style={{ height }} onClick={() => setCourseChoices(group.courses)}><strong>{group.courses.length} 門同時段課程</strong><span>{group.courses.map((course) => course.name).join(' · ')}</span></button></div>;
            })}
          </div>)}
        </div></div>
        <p className="data-note"><Info size={14} /> 此為 115-1 班級課表，不包含個人跨系選修、通識及加退選結果。</p>
      </section>
    </main>
    ...
    {classPickerOpen && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="class-title"><div className="modal class-modal"><div className="modal-icon"><Users size={23} /></div>{classConfirmed && <button className="modal-close" onClick={() => setClassPickerOpen(false)} aria-label="關閉"><X size={20} /></button>}<span className="eyebrow">STATIC SCHEDULE</span><h2 id="class-title">選擇你的班級</h2><p>搜尋系所、年級或班別；選好後會保存在這台裝置，下次自動載入。</p><label className="search-box"><Search size={18} /><input value={classQuery} onChange={(event) => setClassQuery(event.target.value)} placeholder="搜尋班級，例如：電機三" /></label><div className="class-list">{filteredClasses.map((name) => <button key={name} onClick={() => chooseClass(name)} className={classConfirmed && name === selectedClass ? 'selected' : ''}><span><strong>{name}</strong><small>115-1 班級課表</small></span>{classConfirmed && name === selectedClass && <Check size={18} />}</button>)}{!filteredClasses.length && <div className="no-result">找不到符合的班級，請嘗試其他關鍵字</div>}</div></div></div>}

    {courseChoices && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="course-options-title"><div className="modal conflict-modal"><button className="modal-close" onClick={() => setCourseChoices(null)} aria-label="關閉"><X size={20} /></button><span className="eyebrow">COURSE OPTIONS</span><h2 id="course-options-title">同時段有 {courseChoices.length} 門課</h2><p>這個班級在相同時段開設多門選修，請選擇要查看的課程。</p><div className="class-list conflict-list">{courseChoices.map((course) => <button key={course.id} onClick={() => { setCourseChoices(null); setDetail(course); }}><span><strong>{course.name}</strong><small>{weekdays[course.day - 1]} {periods[course.start - 1].start}–{periods[course.end - 1].end} · {course.room}</small></span><MapPin size={17} /></button>)}</div></div></div>}
